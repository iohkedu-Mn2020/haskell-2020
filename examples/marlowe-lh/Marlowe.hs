...
{-@ type Amount = { i : Integer | i >= 0 } @-}
{-@ type PosAmount = { i : Integer | i > 0 } @-}
...
data ReduceWarning =
    ReduceNoWarning
  | ReduceNonPositivePay (AccountId) (Payee) ({ i : Integer | i <= 0 })
  | ReducePartialPay (AccountId) (Payee) (Amount) (Amount)
...
{-@ type AccountMap = Map AccountId (Map Token PosAmount) @-}
...
evalScale factor integer =
      let
        ...
        nn     = integer * n
        (q, r) = quotRem nn (if d == 0 then 1 else d)
      in
...
{-@ refundOne :: AccountMap -> Maybe (Payment, AccountMap) @-}
...
{-@ ifThenElse :: b : Bool -> { t : a | b } -> { e : a | not b } -> a @-}
...
{-@ moneyInAccount :: AccountId -> Token -> AccountMap -> Amount @-}
...
{-@ giveMoney :: Payee -> Amount -> Token -> AccountMap -> (ReduceEffect, AccountMap) @-}
